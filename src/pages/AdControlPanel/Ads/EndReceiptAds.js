...
function EndReceiptAds() {
  ...
  const handleGetListFunc = async () => {
    ...
    try {
      let response = await getAdsListServ({ category: "end_receipt" });
  ...
  const handleDeleteAds = async (id) => {
    ...
    if (confirmed) {
      try {
        let response = await deleteAdsServ({ category: "end_receipt", id: id });
        if (response?.data?.statusCode == "200") {
          toast.success(response?.data?.message);
          handleGetListFunc();
        } else {
    ...
    <div className="main_layout  bgBlack d-flex">
      ...
      <section className="section_layout" style={{ marginLeft: globalState?.isFillSidebarWidth100 ? "260px" : "80px" }}>
        ...
        <div className="tableMain">
          <TableNav tableNav={tableNav} selectedItem="End Receipt" sectedItemBg="#F3F3F3" selectedNavColor="#000" />
            ...
            <div className="modal-content" style={{borderRadius:"30px"}}>
              ...
              <div className="modal-body p-3">
                <div
                  style={{
                    backgroundSize: "100%",
                    backgroundImage: "url(/icons/priceAndCityIcons/endRecipent.png)",
                    height: "585px",
                    width: "295px",
                    wordWrap: "break-word",
                    whiteSpace: "pre-wrap",
                ...
                >
                  <img
                    src={preview?.image}
                    style={{ height: "125px", position: "relative", top: "24px", width: "80%", borderRadius: "12px" }}
                  />
                </div>
                <div className="d-flex justify-content-center">
                  <button
