...
function UserAds() {
  ...
  const handleGetListFunc = async () => {
    ...
    try {
      let response = await getAdsListServ({ category: "user" });
  ...
  const handleDeleteAds = async (id) => {
    ...
    if (confirmed) {
      try {
        let response = await deleteAdsServ({ category: "user", id: id });
        if (response?.data?.statusCode == "200") {
          toast.success(response?.data?.message);
          handleGetListFunc();
        } else {
  ...
  useEffect(() => {
   ...
   return(
    <div className="mainBody">
      <NewSidebar selectedItem="Ad Control Panel" />
            ...
            <div className="modal-content" style={{ borderRadius: "30px" }}>
              ...
              <div className="modal-body p-3">
                <div
                  style={{
                    backgroundSize: "100%",
                    backgroundImage: "url(/icons/priceAndCityIcons/userAddPhone.png)",
                    height: "585px",
                    width: "295px",
                    wordWrap: "break-word",
                    whiteSpace: "pre-wrap",
                ...
                >
                  <img
                    src={preview?.image}
                    style={{ height: "150px", position: "relative", top: "24px", width: "80%", borderRadius: "12px" }}
                  />
                </div>
                <div className="d-flex justify-content-center">
                  <button
