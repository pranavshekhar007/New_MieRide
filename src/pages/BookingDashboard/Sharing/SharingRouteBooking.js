...
import {
  getGroupRouteRecordServ,
  assignDriverManuallyServ,
  sendBookingToManual,
  mergeReverseGroupServ,
  getBookingListForReverceGroup,
  unlinkGroupServ,
...
function SharingRouteBooking() {
  ...
  const handleGetListFunc = async () => {
    ...
    try {
      let response = await getGroupRouteRecordServ(payload);
  ...
  const [moveToManualLoader, setMoveToManualLoader] = useState(null);
  const addGroupToManualFunc = async (id) => {
    setMoveToManualLoader(id);
    try {
      let response = await sendBookingToManual({ route_id: id });
      if (response?.data?.statusCode == "200") {
    ...
    setMoveToManualLoader(null);
    setPopupData(false);
  };
  const [popupData, setPopupData] = useState(null);
  const [submitLoaderId, setSubmitLoaderId] = useState();
  const assignDriversFunc = async (id) => {
    setSubmitLoaderId(id);
    try {
      ...
      setPopupData({
        id,
        message:
          "Unfortunately, we couldn’t find a driver matching the availability for this booking group. Would you like to move this group to manual processing?",
      });
      setSubmitLoaderId("");
    }
  };
  ...
  const fetchGroupId = async (booking_id) => {
    ...
    try {
      let response = await getBookingListForReverceGroup({ booking_id });
      if (response?.data?.statusCode == "200") {
        toast.success(response?.data?.message);
        if (
  ...
  const assignBookingToExistingGroup = async () => {
    ...
    try {
      let response = await mergeBookingToExistingGroupServ(
        manualAssignFormData
      );
      if (response?.data?.statusCode == "200") {
        toast.success(response?.data?.message);
  ...
  return (
    <div className="mainBody">
                        ...
                        <tbody className="bgWhite w-100 routeMinTBody">
                          <tr>
                            ...
                            <td>
                              <div>
                                <h5>No. of Person</h5>
                                <h6>{value?.total_number_of_people}</h6>
                              </div>
                            </td>
            ...
            <div className="modal-content managepopupgroup">
              <div className="modal-body p-0">
                <div className="row m-0 p-0">
                  ...
                  {/* ---------- Right Section (Group Selection + Actions) ---------- */}
      ...
      {popupData && (
        <div className="noDriverPopupContainer">
          <div className="noDriverPopup">
            <h2>No Driver Matches Found</h2>
            <p>{popupData.message}</p>

            <div className="popupBtnRow">
              <button
                className="popupBlackBtn"
                onClick={() => addGroupToManualFunc(popupData.id)}
              >
                Move To Manual
              </button>
                            ...
                            <div className="d-flex justify-content-center">
                              ...
                              >
                                <div className="d-flex justify-content-center w-100 px-4">
                                  <div>Group ID :- </div>
                                  <div className="ms-1">{value?.group_id}</div>
                                </div>
                              </div>
                            </div>
                            ...
                            >
                              <div className="col-6 p-1">
                                {submitLoaderId == value?.id ? (
                                  ...
                                  <button
                                    className="sharingRouteButton w-100"
                                    onClick={() => assignDriversFunc(value?.id)}
                                  >
                                    <div className="d-flex justify-content-center align-items-center">
                                      <div className="">Assign Driver</div>
                                    </div>
                                  </button>
                                )}
                              </div>
                              ...
                              <div className="col-12 mt-2  p-1">
                                {moveToManualLoader == value?.id ? (
                                  ...
                                  >
                                    <div className="d-flex justify-content-center align-items-center">
                                      <div className="">Send To Manual</div>
                                    </div>
                                  </button>
                                )}
                              </div>
      ...
      {popupData && (
        ...
        >
          <div className="modal-dialog" style={{ width: "600px" }}>
            <div className="modal-content  w-100  p-4 noDriverPopUp">
              ...
              <div className="modal-body py-3 px-0">
                <h6 className="mb-5">No Driver Matches Found</h6>
                <p>
                  “Unfortunately, we couldn’t find a driver matching the
                  availability for this booking group. Would you like to move
                  this group to manual processing?”
                </p>
                <div className="d-flex justify-content-between mt-5">
                  <button onClick={() => addGroupToManualFunc(popupData?.id)}>
                    Move To Manual
                  </button>
                  <button
                    style={{
              ...
              <div className="modal-body">
                ...
                >
                  <div className="w-100 px-2">
                    ...
                    {manualPopupDetails?.map((v, i) => {
                      return (
                        <button
                          ...
                          style={{
                            background:
                              manualAssignFormData?.booking_id == v?.id
                                ? "#139F01"
                                : "#CD4F41",
                          }}
                          onClick={() => {
                    ...
                    {manualAssignFormData.booking_id &&
                    manualAssignFormData?.group_id ? (
                      <div className="mx-5 px-2" style={{ marginTop: "-10px" }}>
                        {" "}
                        <button
                          className=" mb-2 "
                          style={{ opacity: moveBtnLoader ? "0.5" : "1" }}
                          onClick={
                            !moveBtnLoader && assignBookingToExistingGroup
                          }
                        >
                          {moveBtnLoader ? "Move ..." : "Move"}{" "}
                        </button>
                      </div>
                    ) : (
                      <div className="mx-5 px-2" style={{ marginTop: "-10px" }}>
                        {" "}
                        <button className=" mb-2 " style={{ opacity: "0.5" }}>
                          Move
                        </button>
                      </div>
                    )}
