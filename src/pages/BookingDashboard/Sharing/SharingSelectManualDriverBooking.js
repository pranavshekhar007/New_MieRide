...
function SharingSelectManualDriverBooking() {
  ...
  const handleGetUserListFunc = async () => {
    setShowSkelton(true);
    try {
      const response = await getDriverNewListServ({
        ...payload,
                ...
                <table className="table mb-0">
                  ...
                  {showSkeltonForDetails
                    ...
                    : availabilityList?.map((v, i) => {
                        const driver = v?.driver_details || v;

                        return (
                          <tbody key={driver?.id || i}>
...
export default SharingSelectManualDriverBooking;
