...
function SharingUpcomingGroupBooking() {
      ...
      if (response?.data?.statusCode == "200") {
        ...
        if (response?.data?.tip_amount) {
          setTipForm({
            ...
            total_trip_amount: response?.data?.total_trip_amount,
            booking_driver_earning: response?.data?.booking_driver_earning,
            decreas_amount: response?.data?.tip_amount,
          });
        }
  ...
  const newAssignBookingToExistingGroupWithTip = async () => {
    setMoveBtnLoader(true);
    try {
      let finalPayload;
      if (tipForm?.tip_status == "keep") {
        finalPayload = {
          tip_status: "keep",
          booking_id: tipForm.booking_id,
          group_id: tipForm.group_id,
        };
      } else if (tipForm?.tip_status == "change") {
        finalPayload = {
          tip_status: "change",
          booking_id: tipForm.booking_id,
          group_id: tipForm.group_id,
          new_amount: tipForm?.new_amount,
        };
      } else {
        finalPayload = {
          tip_status: "debit",
          booking_id: tipForm.booking_id,
          group_id: tipForm.group_id,
          decreas_amount: tipForm?.decreas_amount,
        };
      }
      let response = await assignBookingToExistingGroupServ(finalPayload);
      ...
      <div className="contentLayout">
        <div className="bgWhite borderRadius30 p-4 minHeight100vh">
          <div className="sticky-top bgWhite">
            ...
            <SecondaryTopNav
              navItems={tableNav}
              selectedNav="Upcoming Group"
              navBg="#E5E5E5"
              navColor="#1C1C1C"
              selectedNavBg="#353535"
              selectedNavColor="#fff"
            />
            <div className="upcomingDateFilter d-flex mt-4">
              <p
                onClick={() => setPayload({ ...payload, booking_date: "" })}
                style={{ cursor: "pointer" }}
                className={
                  " " +
                  (payload?.booking_date == "" ? " bgDark textSuccess" : " ")
                }
              >
                All
              </p>
              {upcomingDates?.map((v, i) => {
                return (
                  <p
                    style={{ cursor: "pointer" }}
                    className={
                      " " +
                      (payload?.booking_date == v?.payloadDate
                        ? " bgDark textSuccess"
                        : " ")
                    }
                    onClick={() =>
                      setPayload({ ...payload, booking_date: v?.payloadDate })
                    }
                  >
                    {v?.viewDate}
                  </p>
                    ...
                    <div className="d-flex justify-content-center mt-4 mb-3">
                      {switchType == "personal" ? (
                        <button
                          ...
                          onClick={() => {
                            setShowSwitchPopupId(switchFormData?.booking_id);
                            setSwitchFormData({
                              booking_id: "",
                              group_id: "",
                              pickup_datetime: "",
            ...
            <div className="modal-content tipPopUp">
              ...
              <div className="modal-body tipPopUpBody">
                    ...
                    <div>
                      <div>
                        <input
                          ...
                          checked={tipForm?.tip_status == "keep"}
                          onChange={() => {
                            setTipForm({ ...tipForm, tip_status: "keep" });
                          }}
                        />
                        <label>Keep Tip</label>
                      </div>
                      <div className="d-flex justify-content-between align-items-center">
                        <div>
                          <input
                            type="radio"
                            style={{ width: "20px" }}
                            checked={tipForm?.tip_status == "change"}
                            onChange={() => {
                              setTipForm({ ...tipForm, tip_status: "change" });
                            }}
                          />
                          <label>Change Tip</label>
                        </div>
                      ...
                      <div>
                        <input
                          ...
                          checked={tipForm?.tip_status == "debit"}
                          onChange={() => {
                            setTipForm({ ...tipForm, tip_status: "debit" });
                          }}
                        />
                        <label>Remove Tip</label>
                      </div>
                    </div>
                    <p className="mt-4" style={{ fontSize: "16px" }}>
                      Total Driver Payout :{" "}
                      {tipForm?.tip_status == "keep" ? (
                        <b>
                          $
                      ...
                      ) : tipForm?.tip_status == "change" ? (
                        <b>
                          $
                          {parseInt(tipForm?.total_trip_amount || 0) +
                            parseInt(tipForm?.new_amount || 0) +
                            parseInt(tipForm?.booking_driver_earning || 0)}
                        </b>
                      ) : tipForm?.tip_status == "debit" ? (
                        <b>
                          $
                          {parseInt(tipForm?.total_trip_amount || 0) +
                            parseInt(tipForm?.tip_amount || 0) +
                            parseInt(tipForm?.booking_driver_earning || 0) -
                            parseInt(tipForm?.decreas_amount || 0)}
                        </b>
                      ) : (
                        <b>
                          $
                ...
                <div className="d-flex justify-content-center mt-4">
                  ...
                  <button className="ms-2 " style={{ background: "#1C1C1C" }} onClick={()=>newAssignBookingToExistingGroupWithTip()}>
                    Confirm
                  </button>
                </div>
              </div>
...
export default SharingUpcomingGroupBooking;
