...
function BroadcastUserPromptBookingSharing() {
  const { setGlobalState, globalState } = useGlobalState();
  const [payload, setPayload] = useState({
    per_page: 10,
    page_no: 1,
    user_type: "user",
    type: "sharing_booking",
    category: "prompt",
  });
  ...
  const tableNav = [
    ...
    },
    {
      ...
      path: "/broadcast-user-prompt-booking-personal",
    },
    {
      name: "Deals",
      path: "/broadcast-user-prompt-deals",
    },
    {
      name: "Holidays",
      path: "/broadcast-user-prompt-holidays",
    },
    {
      name: "App Update",
      path: "/broadcast-user-prompt-app-update",
    },

    {
      name: "Maintenance",
      path: "/broadcast-user-prompt-maintenance",
    },
  ];
  ...
  const handleUpdateNotification = async () => {
    ...
    formData.append("id", editNotificationForm?.id);
    formData.append("title", editNotificationForm?.title);
    formData.append("message", editNotificationForm?.message);
    formData.append("scheduled_date", editNotificationForm?.scheduled_date);
    formData.append("scheduled_times", [JSON.parse(editNotificationForm?.scheduled_times)[0]]);
    formData.append("image", editNotificationForm?.image);
    try {
      // let response = await updateScheduleNotification({
      //   ...editNotificationForm,
      //   scheduled_times: [JSON.parse(editNotificationForm?.scheduled_times)[0]],
      // });
      let response = await updateScheduleNotification(formData)
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setEditNotificationForm({...editNotificationForm, imgPrev:URL.createObjectURL(file), image:file})
    }
  };
  return (
      ...
      >
        ...
        <div className="sticky-top bg-light" style={{ paddingTop: "45px" }}>
          ...
          <BroadcastNotificationNav
            ...
            selectedItem="Booking - Sharing"
            sectedNavBg="#353535"
            selectedNavColor="#fff"
            isItemMoreThen8={true}
            payload={{ user_type: "user", type: "booking_sharing" }}
                          ...
                          <tr className="bg-light mb-2">
                            ...
                            >
                              <button
                                ...
                                onClick={() => setEditNotificationForm({...v, imgPrev:v?.image})}
                              >
                                Edit
                              </button>
                            </td>
          ...
          <div className="modal-dialog">
            ...
            >
              <div className="modal-body p-0" style={{ fontFamily: "poppins" }}>
                ...
                <div className="d-flex justify-content-between align-items-center p-4 notificationFormContent" style={{ border: "20px", position: "relative", top: "-20px" }}>
                  <div className=" w-100 me-5" >
                    ...
                    />
                     <div
      className="imageUploadDiv text-center my-3 cursor-pointer"
    ...
    >
      ...
      />
      {editNotificationForm?.imgPrev ? <img src={editNotificationForm?.imgPrev} style={{height:"40px"}} alt="Upload" />:<img src="/icons/grayImage.png" alt="Upload" />}
      {/* <img src="/icons/grayImage.png" alt="Upload" /> */}
      <p>Drop your image here, or <b>Browse</b></p>
      <span>Support : TIFF or PDF format (1000 * 1000px required)</span>
      <div>
        <button
          className="mt-2"
          onClick={(e) => {
            e.stopPropagation(); // prevent button from triggering file input
            // your download logic
            alert("Coming Soon");
          }}
        >
          Download Template
                    ...
                    <div className="d-flex justify-content-start notificationFormActionBtn mt-4">
                      <button
                  ...
                  <div className="pnoneIconDiv">
                    <img src="/icons/blackPhoneBg.png" />
                    <div className="phoneNotificationInnerContent mx-3">
                      <div style={{ background: "#d0ff13", borderRadius: "20px 20px 0px 0px" }} className="py-3 px-3 pb-4">
                        <h5>{editNotificationForm?.title}</h5>
                        <p>{editNotificationForm?.message}</p>
                      </div>
                      <div>
                        <img src={Image_Base_Url + editNotificationForm?.image} className="img-fluid" />
                      </div>
                      <div className="d-flex justify-content-center closeDiv">
                        <img src="/icons/yellowCloseIcon.png" onClick={() => setEditNotificationForm(null)} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="modal-body p-0" style={{ fontFamily: "poppins" }}>
                ...
                <div className="d-flex justify-content-between align-items-center  " >
                  ...
                  <div className="pnoneIconDiv">
                    ...
                    <div className="phoneNotificationInnerContent mx-3">
                      <div style={{ background: "#d0ff13", borderRadius: "20px 20px 0px 0px" }} className="py-3 px-3 pb-4">
                        <h5>{previewDetails?.title}</h5>
                        <p>{previewDetails?.message}</p>
                      </div>
                      <div>
                        <img src={Image_Base_Url + previewDetails?.image} className="img-fluid" />
                      </div>
                      <div className="d-flex justify-content-center closeDiv">
                        <img src="/icons/yellowCloseIcon.png" onClick={() => setPreviewDetails(null)} />
                      </div>
                    </div>
                  </div>
                </div>
...
export default BroadcastUserPromptBookingSharing;
