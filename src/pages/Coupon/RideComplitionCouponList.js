...
import { getCouponListServ,  updateCouponServ } from "../../services/coupon.service";
import { useParams, useNavigate } from "react-router-dom";
import { toast } from "react-toastify";
import NoRecordFound from "../../components/NoRecordFound";
import { Image_Base_Url } from "../../utils/api_base_url_configration";
function RideComplitionCouponList() {
          ...
          >
            <div style={{ margin: "20px 10px" }}>
              <div className="row my-4">
                <div className="col-2">
                  <button
                    style={{ borderRadius: "20px", border: payload?.category_id == 7 ? "2px solid #FF4500":"none"}}
                    className="btn-primary btn w-100"
                    onClick={()=>setPayload({...payload, category_id:"7"})}
                  >
                   Driver 10
                  </button>
                </div>
                <div className="col-2">
                  <button
                    style={{ borderRadius: "20px", border: payload?.category_id == 8 ? "2px solid #FF4500":"none" }}
                    className="btn-success btn w-100"
                    onClick={()=>setPayload({...payload, category_id:"8"})}
                  >
                    Driver Free 5
                  </button>
                </div>
                        ...
                        return (
                          <>
                            <tr className="bg-light mb-2">
                              ...
                              <td>{v?.coupon_code}</td>
                              <td>{v?.category_details?.applicable_benefits}</td>
                              <td>{v?.valid_till}</td>
            ...
            <div className="modal-content  w-100 " style={{ padding: "20px" }}>
              ...
              <div className="modal-body p-0">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditFormData({ ...editFormData, valid_till: e?.target.value })
                  }
                />
                ...
                {editFormData?.valid_till && editFormData?.status  ? (
                  submitLoader ? (
                    <button
...
export default RideComplitionCouponList;
