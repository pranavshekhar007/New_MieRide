  ...
  return (
    <div className="main_layout  bgBlack d-flex">
      ...
      >
        {/* table List started */}
        <div className="">
          <TableNav tableNav={tableNav} selectedItem="Booking" sectedItemBg="#F3F3F3" selectedNavColor="#000" />
          <div className="tableBody py-2 px-4 borderRadius50All" style={{ background: "#F3F3F3" }}>
            <div style={{ margin: "20px 10px" }}>
              ...
              {globalState?.notificationList
                ?.filter((v) => {
                  return v.notifiable_type == "Booking";
                })
                ?.map((v, i) => {
                  return <NotificationBox v={v} bg= "#139F01"  color="#fff"/>
                })}
                {globalState?.notificationList
                ?.filter((v) => {
                  return v.notifiable_type == "Booking";
