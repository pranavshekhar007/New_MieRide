  ...
  return (
    <div className="main_layout  bgBlack d-flex">
      ...
      >
        {/* table List started */}
        <div className="">
          <TableNav tableNav={tableNav} selectedItem="Funds" sectedItemBg="#F3F3F3" selectedNavColor="#000" />
          <div className="tableBody py-2 px-4 borderRadius50All" style={{ background: "#F3F3F3" }}>
            <div style={{ margin: "20px 10px" }}>
              ...
              {globalState?.notificationList
                ?.filter((v) => {
                  return v.notifiable_type == "Fund";
                })
                ?.map((v, i) => {
                return (
                  
                   <NotificationBox v={v} bg="#FFDE03" color="#000" />
                );
              })}
              {globalState?.notificationList
                ?.filter((v) => {
