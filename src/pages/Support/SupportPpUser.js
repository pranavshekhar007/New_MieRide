...
function SupportPpUser() {
  ...
  const handleListPrivacyPolicyFunc = async () => {
    ...
    try {
      let response = await getPrivacyPolicyServ({ type: "user" });
    ...
    <div className="main_layout  bgBlack d-flex">
      ...
      <section className="section_layout" style={{marginLeft :globalState?.isFillSidebarWidth100 ? "260px": "80px" }}>
        ...
        <div className="tableMain">
          <TableNav tableNav={tableNav} selectedItem="User" sectedItemBg="#F7F6FF" />
          <div className="tableBody py-2 px-4 borderRadius50exceptTopLeft" style={{ background: "#F7F6FF" }}>
            <div className="row marginY35">
              <div className="col-6">
                <div className="">
                  <h2 className="tcHeadind mb-4 ms-2">
                    <span style={{ color: "#ca0361" }}>User</span> Privacy Policy
                  </h2>
                  {showSkelton
                    ? [1, 2, 3, 4]?.map((v, i) => {
                        return (
