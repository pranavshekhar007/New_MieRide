...
function SupportTcDriver() {
  ...
  const handleGetTcFunc = async () => {
    try {
      let response = await getTermsAndConditionServ({ type: "driver" });
      if (response?.data?.statusCode == "200") {
    ...
    <div className="main_layout bgBlack d-flex">
      ...
      <section className="section_layout" style={{marginLeft :globalState?.isFillSidebarWidth100 ? "260px": "80px" }}>
        ...
        <div className="tableMain">
          <TableNav tableNav={tableNav} selectedItem="Driver" sectedItemBg="#F1F1F1" />
...
export default SupportTcDriver;
